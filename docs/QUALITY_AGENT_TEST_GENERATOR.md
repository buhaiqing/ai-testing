...
#### 2.1 需求追溯与覆盖原则（新增 - 四元模型）

| 约束编号 | 约束名称 | 约束描述 | 验证方式 | 违规处理 |
|---------|---------|---------|---------|---------|
| **C-000** | 需求追溯 | 每个测试用例必须追溯到原始需求文档中的具体需求点 | 追溯矩阵验证 | 拒绝生成 |
| **C-001** | 需求覆盖 | 测试套件必须覆盖需求文档中定义的所有关键业务场景 | 需求覆盖分析 | 拒绝生成 |
| **C-002** | 意图对齐 | 生成的测试必须准确反映测试用例文案的验证意图 | 意图图谱匹配 | 拒绝生成 |
| **C-003** | 代码对应 | 测试必须针对源代码中实际存在的功能/接口/组件 | AST 验证 | 拒绝生成 |
| **C-004** | 断言准确 | 断言条件必须精确表达预期结果，无歧义 | 断言语义分析 | 拒绝生成 |
| **C-005** | 数据真实 | 测试数据必须符合业务规则和类型约束 | 数据校验器 | 拒绝生成 |
...
| v1.8 | 2026-04-23 | **架构演进**：将统领性纲要从「三元模型」升级为「四元模型」，显式引入需求文档(Req)作为核心要素，新增需求追溯与覆盖验证机制，扩展质量保证关卡至L8，新增需求相关约束规则 |
| v1.7 | 2026-04-23 | **重大更新**：新增统一术语表章节，包含8大术语类别、48+条核心术语定义、术语关系图谱、命名规范和禁用术语清单，作为文档的基础性先导内容 |
| v1.6 | 2026-04-23 | **重大更新**：新增代码生成约束性条件与刚性原则章节（0.6），包含四大约束维度（正确性/可维护性/可执行性/安全性）、32条具体约束规则、约束验证执行机制和应用矩阵 |
