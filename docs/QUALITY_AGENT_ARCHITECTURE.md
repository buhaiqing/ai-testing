...
| **关联文档** | [AGENTS.md](../AGENTS.md), [TERMINOLOGY.md](./TERMINOLOGY.md), [VERSIONING.md](./VERSIONING.md), [DYNAMIC_THRESHOLD_INCENTIVE_COMPATIBILITY.md](./DYNAMIC_THRESHOLD_INCENTIVE_COMPATIBILITY.md) |
...
本文档详细阐述质量智能体的三大核心架构组件：

| 组件 | 解决的问题 | 核心技术 | 成熟度 |
|-----|-----------|---------|--------|
| **去中心化共识架构** | 单点故障、决策可靠性 | Raft 共识协议 | 业界成熟 |
| **CQRS + Event Sourcing** | 状态管理、审计追溯 | 事件驱动架构 | 业界成熟 |
| **Self-Play 验证机制** | 测试代码质量验证 | 对抗式验证 | 创新实践 |

---

## 二、Harness Engineering 集成架构
...
| 角色 | 职责描述 | 典型使用者 |
|------|---------|-----------|
| **System** | 系统内部服务账号，用于 Agent 间通信和基础设施操作 | 消息队列、数据库、Raft 共识节点 |
...
> **说明**：去中心化架构下，原 Orchestrator 的任务分发和策略决策功能由 Raft 共识层 + Event Bus 替代。
...
> **说明**：去中心化架构下，原 Orchestrator 的权限已重新分配：
> - 任务分发 → Raft Leader（共识决策）
> - 策略决策 → 共识投票（多数节点同意）
> - 状态同步 → Event Bus（事件发布-订阅）
...
在已确定的去中心化共识架构（Raft）和 CQRS + Event Sourcing 基础上，系统还面临以下核心挑战：
...
| 集成点 | 动态阈值机制 | 激励相容机制 |
|-------|-------------|-------------|
| **Raft 共识层** | Leader 节点维护全局阈值状态 | 信誉分数参与共识投票权重 |
...
| 消息类型 | 方向 | 说明 |
|---------|------|------|
| **TASK_ASSIGNMENT** | Raft Leader → Agent | 任务分配（通过共识日志） |
| **TASK_RESULT** | Agent → Raft Leader | 任务结果回报 |
| **HEARTBEAT** | Agent → Raft Leader | 健康心跳（每30s） |
| **STATUS_UPDATE** | Agent → Raft Leader | 状态变更通知 |
| **COORDINATION** | Agent ↔ Agent | 协同工作（如数据共享） |
| **ERROR_REPORT** | Agent → Raft Leader | 异常报告 |

> **说明**：去中心化架构下，消息路由通过 Raft Leader 进行，不再有独立的 Orchestrator。
...
## 七、多 Agent 协作核心组件架构设计

### 7.1 去中心化共识架构（Raft 协议）

#### 7.1.1 设计目标

| 目标 | 说明 | 衡量标准 |
|-----|------|---------|
| **高可用性** | 系统无单点故障 | 可用性 > 99.9% |
| **强一致性** | 所有节点看到相同的状态 | 数据一致性 100% |
| **自动容错** | 节点故障自动检测和恢复 | 故障恢复时间 < 30s |
| **水平扩展** | 支持动态增加节点 | 新增节点时间 < 5分钟 |
...
│   - Candidate → Leader: 获得集群多数节点投票                      │
│   - Candidate → Follower: 发现新 Leader 或选举超时                │
│   - Leader → Follower: 发现更高任期的 Leader                      │
│                                                                 │
│   安全性保证：                                                    │
│   - 任意时刻最多只有一个 Leader（ per term ）                     │
│   - 已提交的日志条目不会被覆盖或删除                              │
│   - 日志顺序一致性：所有节点看到的日志顺序相同                     │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 角色 | 职责 | 数量要求 | 故障影响 |
|-----|------|---------|---------|
| **Leader** | 接收客户端请求，复制日志到 Follower，提交已复制日志 | 1 个（任意时刻） | Leader 故障触发选举，服务中断 < 30s |
| **Follower** | 接收并持久化 Leader 的日志，参与选举投票 | 奇数个（2, 4, 6...） | 少数 Follower 故障不影响服务 |
| **Observer** | 只接收日志复制，不参与选举（扩展读能力） | 任意数量 | 无影响，可随时增减 |

#### 11.1.4 日志复制流程

```
Client (Agent)          Leader                  Follower
     │                    │                        │
     │── Request ────────▶│                        │
     │                    │── AppendEntries RPC ──▶│
     │                    │   {term, prevLogIndex, │
     ...
     │                    │   {term, success}      │
     │                    │                        │
     │                    │── AppendEntries RPC ──▶│ (下一个 Follower)
     │                    │                        │
     │◀── Response ───────│                        │
     │   {success}        │                        │
     │                    │                        │
...
│                                                                 │
│   集成要点：                                                     │
│   1. Agent 作为 Raft 的客户端，向 Leader 提交状态变更请求         │
│   2. Leader 将变更作为日志条目复制到所有节点                      │
│   3. 当日志在多数节点确认后，状态变更生效                         │
│   4. Agent 通过 Event Bus 订阅状态变更事件                        │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 参数 | 默认值 | 说明 | 调优建议 |
|-----|-------|------|---------|
| `election_timeout_min` | 150ms | 选举超时最小值 | 网络延迟高时适当增加 |
| `election_timeout_max` | 300ms | 选举超时最大值 | 通常为 min 的 2 倍 |
| `heartbeat_interval` | 50ms | Leader 心跳间隔 | 应小于 election_timeout_min |
| `max_log_entries_per_rpc` | 100 | 单次 RPC 最大日志条目数 | 网络带宽高时可增加 |
| `snapshot_interval` | 10000 | 快照间隔（日志条目数） | 日志增长快时减小 |

---

### 11.2 CQRS + Event Sourcing 事件层
...
| **AgentAssigned** | Agent 分配 | `task_id`, `agent_id`, `assigned_at` | Raft Leader 分配任务 |
...
| v2.2.0 | 2026-04-26 | **架构简化**：移除 Orchestrator 角色，任务分发和策略决策功能由 Raft 共识层 + Event Bus 替代 | - |
| v2.1.0 | 2026-04-26 | **架构升级**：新增去中心化共识架构（Raft协议）、CQRS+Event Sourcing事件层、Self-Play验证机制三大核心组件的详细架构设计 | - |
